...
test("rejects malformed and GraphQL-error responses through the transport boundary", async () => {
  ...
  await expect(malformed.getProducts(20)).rejects.toMatchObject({ code: "shopify_response_invalid" });
  ...
  await expect(graphqlError.getProducts(20)).rejects.toMatchObject({ code: "shopify_request_failed", retriable: false });
});
