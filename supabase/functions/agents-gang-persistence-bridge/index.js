...
function parseOperation(path, method) {
  let pathname;
  try {
  ...
  if (method === "GET" && READ_TABLES.has(pathname)) {
    return { pathname, scope: pathname === "/approval_requests" ? "founder" : "operator" };
  }
  if (method === "POST" && (POST_TABLES.has(pathname) || RPC_PATHS.has(pathname))) {
    return { pathname, scope: "operator" };
  }
...
async function sessionIsValid(authorization, scope) {
  try {
